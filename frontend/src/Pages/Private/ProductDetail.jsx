...
const ProductDetail = () => {
    ...
    const { data: { _id, name, bids, location, postStatus, starting_price, starting_time, type_of_waste, weight, imageURL } } = product;
            ...
            <div className='p-12 container mx-auto flex flex-col justify-between lg:flex-row gap-16 lg:items-center'>
                ...
                <div className='flex flex-col gap-4 lg:w-2/4'>
                    ...
                    <div className='mt-12 shadow-xl rounded-xl p-12'>
                        <h2 className='font-text font-extrabold text-4xl underline'>Live Bidding</h2>
                        <div className="flex flex-col gap-2 mt-2">
                            {bids.slice().reverse().map((bid) => (
                                <div key={bid._id} className="flex gap-10 items-center">
                                    <p className="text-lg font-text">Name: {bid.company.fullName}</p>
                                    <p className="text-lg font-text">Bid Amount: RM {bid.price}</p>
                                </div>
                            ))}
                        </div>
                    </div>
